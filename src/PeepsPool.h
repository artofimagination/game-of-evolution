...
public:
    ...
    Peep& getPeep(Coord loc) { return individuals[m_Grid.at(loc)]; }
    const Peep& getPeep(Coord loc) const { return individuals[m_Grid.at(loc)]; }
    // Direct access:
    Peep & operator[](uint16_t index) { return individuals[index]; }
    Peep const & operator[](uint16_t index) const { return individuals[index]; }
    
    void displaySampleGenomes(unsigned count);

private:
    std::vector<Peep> individuals; // Index value 0 is reserved
